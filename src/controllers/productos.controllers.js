...
export const editarProducto = async (req, res) => {
    try {
        ...
        if(!productoBuscado) return res.status(404).json({mensaje: "El producto no fue encontrado!"});

        const productoEditado = await Producto.findByIdAndUpdate(req.params.id, req.body);

        res.status(200).json({mensaje: "El producto fue editado correctamente", productoEditado})
    } catch (error) {
        console.error(error);
        res.status(500).json({
